...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\gohash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\gohash
    // Mac: ~/Library/Application Support/gohash
    // Unix: ~/.gohash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "gohash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gohash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gohashd.pid"));
